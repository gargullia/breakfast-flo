...
static int __mdp_lut_hw_update(struct fb_cmap *cmap, uint32_t *internal_lut)
{
	int offset = IS_ENABLED(CONFIG_FB_MSM_MDP40) ? 0x94800 : 0x93800, i;
	u16 r, g, b;

	if (cmap->start > MDP_HIST_LUT_SIZE ||
	    cmap->len > MDP_HIST_LUT_SIZE ||
	...
	for (i = 0; i < cmap->len; i++) {
		if (internal_lut) {
			r = lut2rgb(internal_lut[i], R_SHIFT);
			g = lut2rgb(internal_lut[i], G_SHIFT);
			b = lut2rgb(internal_lut[i], B_SHIFT);
		...
		MDP_OUTP(MDP_BASE + offset + (0x400 * mdp_lut_i) +
			 cmap->start * 4 + i * 4, last_lut[i]);
...
static inline int mdp_lut_hw_update(struct fb_cmap *cmap)
{
	return __mdp_lut_hw_update(cmap, NULL);
}

static inline int mdp_lut_hw_update_kcal(struct fb_cmap *cmap)
{
	uint32_t qlut_map[256] = {
	...
	return __mdp_lut_hw_update(cmap, qlut_map);
}
...
int mdp_preset_lut_update_lcdc(struct fb_cmap *cmap)
{
	int ret;
	uint32_t out;
	...
	ret = mdp_lut_hw_update_kcal(cmap);
	if (IS_ERR_VALUE(ret))
		goto fail;

	out = inpdw(MDP_BASE + 0x90070);

	/* Write to register twice to trigger panel update */
	MDP_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x7 | out);
	mdp_lut_i = !mdp_lut_i;
	MDP_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x7 | out);
fail:
